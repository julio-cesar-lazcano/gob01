...
class mail_ihce(osv.Model):
    _name = 'mail.ihce'
    
    def send_mail_user(self, cr, uid, ids, titulo, texto, usuario, context=None):
        email_vals = {}
        
        mail_message_obj = self.pool.get('mail.mail')
        user_row = self.pool.get('res.users').browse(cr, uid, usuario)
        user_from = self.pool.get('res.users').browse(cr, uid, uid)
        row = self.browse(cr, uid, ids, context)[0]

        #~ Se le envía correo sólo si lo admite
        if user_row.email_crm:
            if user_row.email:
                subject = titulo
                
                body_text = texto

                email_vals.update({'subject':subject, 'body_html':body_text, 'body':body_text, 'email_to': user_row.email, 'email_from':user_from.email})
...
class mail_mail(osv.Model):
    ...
    _columns = {
        'email_bcc':  fields.char('Cco', help='Carbon copy message recipients'),
    }
