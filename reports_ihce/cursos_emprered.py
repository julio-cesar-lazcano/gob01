...
class reporte_cursos_emprered(osv.osv_memory):
    _name = "reporte.cursos.emprered"
    ...
    def action_create_report(self, cr, uid, ids, context=None):
        ...
        sheet_principal = workbook.add_sheet('Cursos Emprered', cell_overwrite_ok=True)

        # Creamos la Hoja principal
        self.create_principal_sheet(cr, uid, ids, sheet_principal, context)
        # Creamos el nombre del archivo
        name = "Cursos Emprered.xls"
        ...
        data_attach = {
            ...
            'datas': sprint_file,
            'datas_fname': name,
            'description': 'Reporte Cursos Emprered',
            'res_model': 'reporte.cursos.emprered',
            'res_id': ids[0],
        }
    ...
    def create_principal_sheet(self, cr, uid, ids, sheet, context={}):
        ...
        styleG = xlwt.easyxf(('font: height 220, bold 1, color black; alignment: horizontal center; pattern: pattern solid, fore_colour gray25;'))
        ...
        sheet.write_merge(3, 3, 0, 12,("CURSOS EMPRERED"), styleT)
        #TITULOS
        ...
        sheet.write(5, 8, 'Producto', style)
        sheet.write(5, 9, 'Tipo', style)
        sheet.write(5, 10, 'Tema', style)
        sheet.write(5, 11, 'Horas', style)
        ...
        for row in self.pool.get('date.courses').browse(cr, uid, courses_ids, context):
            ...
            for line in self.pool.get('company.line').search(cr, uid, [('course_id','=',row.id)]):
                ...
                sheet.write(m, 0, a, style_n)
                sheet.write(m, 1, row.emprered.name or '', style_n)
                sheet.write(m, 2, ro.parent_id.type or '', style_n)
                sheet.write(m, 3, (ro.parent_id.name) or '', style_n)
                sheet.write(m, 4, ro.name or '', style_n)
                sheet.write(m, 5, ro.sexo or '', style_n)
                sheet.write(m, 6, ro.town.name or '', style_n)
                sheet.write(m, 7, ro.parent_id.sector.name or '', style_n)
                sheet.write(m, 8, ro.idea_commerce or '', style_n)
                sheet.write(m, 9, row.type or '', style_n)
                sheet.write(m, 10, (row.courses_id.name.encode('utf-8') +" -- " + row.name.encode('utf-8')) or '',style_n)
                sheet.write(m, 11, row.hours_training or '', style_n)
                sheet.write(m, 12, mes1 + '-' + str(row.date[0:4]) or '', style_n)
            ...
            for line in self.pool.get('list.new.persons').search(cr, uid, [('course_id','=',row.id)]):
                ...
                sheet.write(m, 0, a, style_n)
                sheet.write(m, 1, row.emprered.name or '', style_n)
                sheet.write(m, 2, '', style_n)
                sheet.write(m, 3, '', style_n)
                sheet.write(m, 4, li.name or '', style_n)
                sheet.write(m, 5, li.sexo or '', style_n)
                sheet.write(m, 6, li.town.name or '', style_n)
                sheet.write(m, 7, '', style_n)
                sheet.write(m, 8, li.idea_commerce or '', style_n)
                sheet.write(m, 9, row.type or '', style_n)
                sheet.write(m, 10, (row.courses_id.name.encode('utf-8') +" -- " + row.name.encode('utf-8')) or '',style_n)
                sheet.write(m, 11, row.hours_training or '', style_n)
                sheet.write(m, 12, mes1 + '-' + str(row.date[0:4]) or '', style_n)
