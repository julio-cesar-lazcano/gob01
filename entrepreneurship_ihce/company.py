...
class companies_ihce(osv.Model):
    ...
    def _get_services(self, cr, uid, ids, field, arg, context=None):
        ...
        for row in self.browse(cr, uid, ids, context=context):
            if len(row.emprendimiento_id) > 0:
                mark = True
                res[row.id] = True
            else:
                res[row.id] = False
    ...
    _columns = {
        'emprendimiento_id': fields.one2many('entrepreneurship.ihce', 'company_id',"Servicio Emprendimiento"),
        'empre_len': fields.function(_get_services, type='boolean', string="Servicios Emprendimiento"),
    }
