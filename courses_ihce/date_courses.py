...
class date_courses(osv.Model):
    ...
    def complete(self, cr, uid, ids, vals, context=None): 
        row = self.browse(cr, uid, ids[0], context=context)
        today = datetime.today()
        mes = today.strftime("%m")
        today2=row.date
        ...
        if mes == mes2: 
            ...
            raise osv.except_osv(_('Advertencia!'), _('Solo puedes modificar tus cursos en fechas aprobadas!'))
...
class company_line(osv.Model):
    ...
    def onchange_company(self, cr, uid, ids, contact, context=None):
        ...
        if contact:
            ...
            #cambio realizado po Viviana Tenorio 18/04/17. NO guardaba company.id en conpany.line
            if row.parent_id.id:
                result['value'].update({'name_company': razon, 'town':row.town , 'phone': row.phone, 'cel_phone': row.cel_phone, 'email': row.email,'company_id': row.parent_id.id,'idea_commerce': row.idea_commerce})
            #cambio realizado po Viviana Tenorio 18/04/17. NO guardaba company.id en conpany.line
            else: 
                result['value'].update({'name_company': razon, 'town':row.town , 'phone': row.phone, 'cel_phone': row.cel_phone, 'email': row.email,'company_id': row.id,'idea_commerce': row.idea_commerce})
        return result
